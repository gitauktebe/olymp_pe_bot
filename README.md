...
- `/admin` — админ-меню (добавление single-вопросов, список вопросов с фильтрами/пагинацией, управление темами)
...
## Добавление вопросов
- Через `/admin` → «➕ Добавить вопрос».
- FSM для типа `single`: выбор темы (или создание новой), ввод `prompt`, ввод вариантов `A/B/C/D`, выбор правильного варианта кнопками, превью и подтверждение сохранения.
- При сохранении в `questions` заполняются поля нового формата: `prompt`, `type='single'`, `options` (jsonb), `answer` (jsonb `{"correct":[N]}`), `is_active=true`.
- Для обратной совместимости дополнительно заполняются legacy-поля `text`, `option1..4`, `correct_option`.

## Импорт вопросов пачкой через админку

- В `/admin` нажмите кнопку «📥 Импорт вопросов (пачкой)».
- Доступно только администраторам.
- Один блок = один вопрос, блоки разделяются строкой `---`.
...
- Вопрос можно задавать как `Q:` или `В:`.
- `TOPIC` и `DIFF` необязательны (если не указаны — сохраняются как `null`).
- `ACTIVE` необязателен, по умолчанию `true`.
- После обработки бот присылает отчёт: сколько добавлено, сколько с ошибками, и первые 3 ошибки.

## Раздел «📚 Вопросы»
- Пагинация по 10 вопросов.
- Фильтры по теме и активности (все / только активные / только неактивные).
- Для каждого вопроса доступны действия: «👁 Открыть», «✅/⛔ Активность», «🗑 Удалить».

## Раздел «🧩 Темы»
- Просмотр списка тем.
- Добавление новой темы.
- Удаление темы только если нет связанных вопросов.
